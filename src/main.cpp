...
#if defined(NDEBUG)
# error "Zenexcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x42bab5b78694e2ba9bc2750107dc949ef67a640f7918cf0baa3972c3317da2ea");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zenexcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zenexcoin Signed Message:\n";
...
{
    // Zenexcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 20000); // Zenexcoin: 20k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Zenexcoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Zenexcoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zenexcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zenexcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc6b5c38d71830540304e3ba9ca754d79428704c105eabdac2125c20ce7f62af7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("048b9f6df4e22f29b33cb131a2d8e6b1e63da338de8ce14b09512f0f409a38e2f27f2588a6ed0beff3dbe15581e7d2c82780217cbacd0297d40538d9299d06a217") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime = 1562122933;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 389535210;

        if (fTestNet)
        {
	    block.nTime    = 1562122897;
            block.nNonce   = 388312566;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1eba7812057d98e51fe8b4ead31c5b94597c00aeae2b7cf0441824b0a7155ae5"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc4, 0xba, 0xd1 }; // Zenexcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZenexcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZenexcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZenexcoinMiner(CWallet *pwallet)
{
    printf("ZenexcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zenexcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZenexcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZenexcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZenexcoinMiner, pwallet));
}
